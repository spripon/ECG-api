"""
ECG‑API  – GPT‑4o Vision ✚ FFT ✚ Couleur  (mai 2025)
© spripon
"""
...
def vision_bbox(img):
    ...
    messages=[{"role":"user","content":[
      {"type":"text","text":(
        ...
        "jaune pâle). Ignore : bande blanche supérieure, bordures latérales, "
        "classeurs, feuilles, fonds. "
        "Renvoie strictement JSON {\"points\":[{\"x\":int,\"y\":int}]} "
        "avec 4‑8 sommets dans l'ordre horaire.")},
      {"type":"image_url",
...
# ───── validation contenu rose/orange ─────
...
def process(img):
    ...
    if not box or not vision_ok(img, box):
        ...
        # encore douteux ?  dernière chance FFT flood
