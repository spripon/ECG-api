"""
ECG-API – extraction de la zone quadrillée + redressement + filtre “argenté”
© spripon – mai 2025
"""
...
VISION_MODEL   = os.getenv("MODEL_NAME", "gpt-4o")    # ou gpt-4o-mini
MAX_DIM        = 1600                                 # limite Vision
...
def warp(img, poly):
    """
    1. masque = polygone rose détecté (évite bords parasites)
    2. Canny + HoughLines → 2 lignes horizontales + 2 verticales principales
    3. intersections → 4 coins (TL TR BR BL)
    4. getPerspectiveTransform → rectangle parfait
       (sinon fallback `warp_old`)
    """
...
def crop_white_top(img):
    ...
    idx = np.argmax(sat.mean(axis=1) > 25)      # S>25 ≈ début zone rose
    return img[idx+5:], idx+5 if idx else 0

# ╭───────── Détection couleur robuste ───╮
def detect_color(img):
    roi, off = crop_white_top(img)
    ...
    ros_idx = np.argmax(ctr[:, 0])               # a* max = rose
    mask = (lbl.reshape(lab.shape[:2]) == ros_idx).astype(np.uint8)*255

    hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)
    mask[hsv[:, :, 1] < 40] = 0                  # retire gris/blanc
    ...
    Amax = cv2.contourArea(max(cnts, key=cv2.contourArea))
    cnts = [c for c in cnts if cv2.contourArea(c) > 0.3*Amax]
    c    = max(cnts, key=cv2.contourArea)

    pts  = cv2.boxPoints(cv2.minAreaRect(c)).astype(int)
    pts[:, 1] += off                                 # remet offset

    xs, ys = pts[:, 0], pts[:, 1]
    M = int(0.05 * max(xs.ptp(), ys.ptp()))          # marge sécurité 5 %
...
def detect_vision(img):
    ...
    messages=[{"role":"user","content":[
        {"type":"text","text":"Localise uniquement la zone quadrillée rose de l’ECG. "
                              "Retourne JSON {points:[{x,y}]}."},
...
def enhance(img):
    ...
    bg   = cv2.medianBlur(exp, 51)                          # enlève ombres
...
def process(img):
    ...
    if not box:
        raise ValueError("zone quadrillée non trouvée")
...
@app.route("/")
def home():
    return "<h3>ECG-API – quadrillage + redressement + filtre argenté</h3>"
