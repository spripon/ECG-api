"""
ECG‑API  –  extraction quadrillage + “filtre iPhone”
© spripon ‑ mai 2025
"""
...
# ───────── détection couleur (rose / rouge pâle) ─────────
...
def detect_vision(img):
    ...
    messages=[{"role":"user","content":[
      {"type":"text","text":"Donne 4 à 8 coins de la zone quadrillée ECG rose."},
...
# ───────── filtre “argenté” (exposition + brillance + ombres) ─────────
def enhance(img):
    # 1) +20 % d’exposition / brillance
    exp = cv2.convertScaleAbs(img, alpha=1.2, beta=20)   # alpha gain, beta offset

    # 2) désaturation 50 %  → look « argenté »
    hsv = cv2.cvtColor(exp, cv2.COLOR_BGR2HSV)
    hsv[:,:,1] = (hsv[:,:,1] * 0.5).astype(np.uint8)
    exp = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)

    # 3) suppression ombres (~+75 %)   ▸ division par fond flouté
    bg   = cv2.medianBlur(exp, 51)
    flat = cv2.divide(exp, bg, scale=255)

    # 4) léger sharpen
    gauss = cv2.GaussianBlur(flat,(0,0),3)
    sharp = cv2.addWeighted(flat, 1.3, gauss, -0.3, 0)
