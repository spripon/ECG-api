...
def detect_vision(img):
    ...
    messages=[{"role":"user","content":[
        {"type":"text","text":"Localise la zone quadrillée ECG rose/orange."},
    ...
    tools=[{"type":"function","function":{"name":"set","parameters":{
        ...
        "required":["points"]}}}]
    try:
        rsp = client.chat.completions.create(model=VISION_MODEL, messages=messages,
                    tools=tools, tool_choice={"type":"function","function":{"name":"set"}}, temperature=0)
...
@app.route('/')
def home():
    return '<h3>ECG‑API couleur + Vision fallback</h3>'
